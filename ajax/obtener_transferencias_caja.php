...
try {
    ...
    // Cualquier fallo (p. ej. una columna faltante porque una migración no se
    // aplicó) debe responder SIEMPRE JSON: si la excepción escapa, el manejador
    // global de pages/infosesion.php devuelve su página HTML de error y el
    // fetch(...).json() del dashboard falla con "JSON.parse: unexpected character".
    error_log('obtener_transferencias_caja: ' . $e->getMessage());
    if (ob_get_length()) { ob_clean(); }
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Error al obtener las transferencias de la caja.']);
    exit();
}

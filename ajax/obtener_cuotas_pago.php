...
if ($is_all) {
    ...
    $where_clause = "WHERE v.id_cliente = ?";
    $params[] = $id_cliente;
}
...
try {
    $sql = "SELECT 
                ...
                (SELECT SUM(monto_original) FROM cuotas_seguimiento WHERE id_venta = v.id) as total_financiado,
                (SELECT SUM(monto_original - monto_pagado) FROM cuotas_seguimiento WHERE id_venta = v.id) as saldo_total
            FROM ventas v
    ...
    foreach ($ventas as $v) {
        ...
        $badge = $esta_pago ? '<span class="badge badge-success">Saldado</span>' : '<span class="badge badge-warning">Pendiente</span>';
        ...
        echo "  <td><b>#{$v['n_documento']}</b></td>
                ...
                <td class='text-center'>
                    <button class='btn btn-primary btn-sm' onclick='verDetalleCuotas({$v['id']}, \"{$v['n_documento']}\")'>

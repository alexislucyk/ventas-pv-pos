    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['venta_action'])) {
        ...
        if (empty($detalle_productos)) {
            ...
            try {
                ...
                if ($es_finalizar) {
                    // CORRECCIÓN: Si es CONTADO, entra el total real. Si es CTA CTE, solo lo que entregó.
...
<head>
    ...
    <style>
        ...
        /* ESTILO VUELTO */
        .vuelto-box { background: #1a1a1a; padding: 15px; text-align: center; margin-bottom: 15px; border-radius: 8px; border: 1px solid #f1c40f; display: none; }
...
<body>
    ...
    <script>
        ...
        function actualizarVuelto() {
            ...
            // Obtenemos el texto del total (ej: "$ 100.00")
            var totalText = document.getElementById('total_venta_display').innerText;
            
            // Limpieza profunda: quitamos todo lo que no sea número, punto o coma
            var totalLimpio = totalText.replace(/[^\d,.]/g, '');
            
            // Convertimos a formato numérico puro (manejando puntos de miles o comas decimales)
            ...
            // Mostramos el vuelto si es contado y el pago es mayor al total
            if (cond === 'CONTADO' && pagoTotal > total && total > 0) {
                ...
                box.style.setProperty("display", "block", "important"); // Forzamos que se vea
            } else {
